...
export default function Dashboard({ onNavigate }: { onNavigate: (p: string, d?: any) => void }) {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          setTotalDoctors(staffData.filter((s: any) => s.role === 'Doctor').length);
        } catch {
          setTotalDoctors(0);
        }
        ...
        const occupied = bedsData.filter((b: any) => b.status === 'Occupied').length;
        ...
        setPendingBillsCount(billingData.filter((b: any) => ['Pending', 'Partial'].includes(b.payment_status)).length);
        ...
        console.error('Offline / fallback to schema defaults:', err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const occupancyPercent = Math.round((bedsOccupied / bedsTotal) * 100);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, padding: '4px 0' }}>
      ...
      <div style={{
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--success)', marginTop: 4, fontWeight: 600 }}>+2 this month <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>· Active roster</span></div>
          </div>
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#f0fdfa', color: '#0d9488', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🩺</div>
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>{admittedCount} admitted</div>
          </div>
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--info-bg)', color: 'var(--info)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>👥</div>
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#f5f3ff', color: '#7c3aed', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🛏️</div>
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Across all doctors</div>
          </div>
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#fffbeb', color: '#d97706', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>📅</div>
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
          <div>
            ...
            <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--danger)', marginTop: 8 }}>{criticalPatients}</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Require attention</div>
          </div>
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#fef2f2', color: '#dc2626', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>⚠️</div>
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
          <div>
            ...
            <div style={{ fontSize: 24, fontWeight: 700, color: 'var(--text)', marginTop: 12 }}>₹{totalBilled.toLocaleString('en-IN')}</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>₹{collectedBilled.toLocaleString('en-IN')} collected</div>
          </div>
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#f0fdf4', color: '#16a34a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>💵</div>
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
          <div>
            ...
            <div style={{ fontSize: 24, fontWeight: 700, color: '#dc2626', marginTop: 12 }}>₹{outstandingBilled.toLocaleString('en-IN')}</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>{pendingBillsCount} pending bills</div>
          </div>
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: '#fef2f2', color: '#dc2626', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🧾</div>
        </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: 'var(--shadow-sm)' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Inpatients</div>
          </div>
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--info-bg)', color: 'var(--info)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>📈</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, flexWrap: 'wrap' }}>
        ...
        <div className="card" style={{ boxShadow: 'var(--shadow-sm)', borderRadius: 'var(--radius-xl)' }}>
          ...
          <div className="card-body" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              { name: 'Rajesh Kumar', uhid: 'UHID-10001', complaint: 'Chest pain, shortness of breath', status: 'Admitted' },
              { name: 'Farhan Ali', uhid: 'UHID-10005', complaint: 'Stroke recovery', status: 'Admitted' },
              { name: 'Sameer Khan', uhid: 'UHID-10009', complaint: 'Fracture surgery follow-up', status: 'Admitted' }
            ].map(p => (
              <div key={p.uhid} style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between',
                padding: '10px 12px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{p.uhid} · {p.complaint}</div>
                  </div>
                </div>
                <span className="badge badge-info" style={{ fontSize: 9.5, padding: '2px 8px' }}>{p.status}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card" style={{ boxShadow: 'var(--shadow-sm)', borderRadius: 'var(--radius-xl)' }}>
          ...
          <div className="card-body" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              { ward: 'ICU', occupied: 5, total: 5, color: '#0d9488' },
              { ward: 'General', occupied: 8, total: 12, color: '#0d9488' },
              { ward: 'Emergency', occupied: 0, total: 4, color: '#cbd5e1' },
              { ward: 'Maternity', occupied: 0, total: 3, color: '#cbd5e1' }
            ].map(w => {
              const pct = Math.round((w.occupied / w.total) * 100) || 0;
              return (
