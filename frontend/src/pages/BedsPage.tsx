  ...
  useEffect(() => {
    if (allocateBed) {
      ...
      apiClient.get('/users/doctors')
        .then(res => setDoctors(Array.isArray(res.data) ? res.data : (res.data?.doctors || [])))
        .catch(() => {});
    }
  }, [allocateBed]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="page-header" style={{ marginBottom: 4 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-primary btn-sm" onClick={() => { setShowAddBedModal(true); setNewBedError(''); }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 5 }}><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            Add Bed
          </button>
          <button className="btn btn-secondary btn-sm" onClick={() => setShowHistory(true)}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 5 }}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
          ...
          <button className="btn btn-secondary btn-sm" onClick={() => fetchBeds(false)} disabled={loading}>
            {loading ? <div className="spinner spinner-sm" /> : (
              <>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 5 }}><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>
                Refresh Status
              </>
            )}
          </button>
      ...
      {loading ? (
        ...
        <div className="empty-state" style={{ padding: '60px 24px' }}>
          <span className="empty-icon"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 4v16"/><path d="M2 8h18a2 2 0 0 1 2 2v10"/><path d="M2 17h20"/><path d="M6 8v9"/></svg></span>
          <h3>No beds found</h3>
      ...
      {editingBed && (
        <div className="modal-overlay" onClick={() => setEditingBed(null)}>
          <div className="modal" style={{ maxWidth: 440 }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <div className="modal-title">Edit Bed Configurations</div>
              <button className="modal-close" onClick={() => setEditingBed(null)}>✕</button>
            </div>
            <form onSubmit={handleEditBed}>
              <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {editBedError && <div className="alert alert-danger">{editBedError}</div>}
