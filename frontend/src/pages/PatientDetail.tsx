                  ...
                  >
                    <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 8, height: '100%' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: 'var(--radius-sm)', background: 'var(--surface-sunken)', color: 'var(--primary)' }}>
                          {doc.file_type?.includes('pdf') ? (
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v20a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                          ) : (
      ...
      <div className="modal" style={{ maxWidth: 460 }} onClick={e => e.stopPropagation()}>
        ...
        {step === 1 && (
          <form onSubmit={handleSendOtp} style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '6px 4px 18px 4px' }}>
            ...
            {error && <div className="alert alert-danger" style={{ fontSize: 12, padding: '8px 12px', margin: 0, display: 'flex', alignItems: 'center', gap: 6 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> {error}</div>}
...
function EditPatientModal({ patient, onClose, onDone }: { patient: any; onClose: () => void; onDone: (p: any) => void }) {
  ...
  function FieldErr({ k }: { k: string }) {
    return fieldErrors[k] ? (
      <div style={{ color:'var(--danger)', fontSize:11, marginTop:3, display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div className="modal" style={{ maxWidth: 480 }} onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={submit}>
          <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {error && <div className="alert alert-danger" style={{ display: 'flex', alignItems: 'center', gap: 6 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> {error}</div>}
