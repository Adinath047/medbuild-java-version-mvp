...
public class EmbeddedRedisConfig {
    ...
    public void startRedis() {
        ...
        if (System.getenv("K_SERVICE") != null || "false".equalsIgnoreCase(System.getenv("SPRING_EMBEDDED_REDIS_ENABLED"))) {
            log.info("[Redis] Cloud Run / Production container detected. Skipping embedded Redis startup.");
            return;
        }

        if (isPortInUse(redisPort)) {
            log.info("[Redis] Redis server is already active on port {}. Skipping embedded Redis startup.", redisPort);
            return;
        }
        ...
        try {
            ...
            redisServer.start();
            log.info("======================================================================");
            log.info("[REDIS] Embedded Redis server started automatically on port {}.", redisPort);
            log.info("======================================================================");
        } catch (Throwable e) {
    ...
    public void stopRedis() {
        if (redisServer != null && redisServer.isActive()) {
            try {
                redisServer.stop();
                log.info("[REDIS] Embedded Redis Server stopped successfully.");
            } catch (Exception e) {
